...
export async function saveOpinionEditAction(_: EditOpinionState, formData: FormData): Promise<EditOpinionState> {
  ...
  if (!draftId) {
    return { error: "No se ha encontrado la columna." };
  }

  if (!titulo || !subtitulo || !entradilla || cuerpo.length === 0 || !autor || !tiempoLectura) {
    return { error: "Completa título, subtítulo, entradilla, autor, tiempo de lectura y al menos un párrafo del cuerpo." };
  }
  ...
  try {
    ...
    revalidatePath("/categoria/opinion");
    revalidatePath(`/articulo/${result.previousSlug}`);
    revalidatePath(`/articulo/${result.slug}`);
  } catch {
    return { error: "No se ha podido guardar la columna de opinión." };
  }

  redirect("/admin/opinion");
}
